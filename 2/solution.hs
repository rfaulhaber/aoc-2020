...
parseRange input = do
  let (h : t : _) =
        ((map (\s -> read s :: Integer)) . (take 2) . (split "-")) $ input
  Range { low = h, high = t }
...
isLineValid Line { range, letter, password } =
  inRange range (charCount letter password)

isLineValid2 :: Line -> Bool
isLineValid2 Line { range, letter, password } = (||)
  (password !! (lowIndex) == letter && password !! (highIndex) /= letter)
  (password !! (lowIndex) /= letter && password !! (highIndex) == letter)
 where
  lowIndex  = (fromIntegral . subtract 1 . low) range
  highIndex = (fromIntegral . subtract 1 . high) range

countValidLines :: (Line -> Bool) -> [Line] -> Integer
countValidLines validator (h : t) = if validator h
  then 1 + countValidLines validator t
  else countValidLines validator t
